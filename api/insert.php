<?php
	...
	function insert_to_foods ($rid, $fid, $fname, $fdesc) {
		if (is_int($rid) && is_int($fid)) {
			...
			$result = pg_execute($db_conn, "food_insert", array($rid, $fid, $fname, $fdesc));
			return $result;
		}
		return false;
	}
	...
	function insert_to_suggestions ($uid, $rid, $fid, $sadded) {
		if (if_int($uid) && if_int(rif) && if_int($fid) && strtotime($sadded)) {
			$query = pg_prepare($db_conn, "suggestion_insert", 'INSERT INTO suggestions VALUES $1, $2, $3, $4');
			$result = pg_execute($db_conn, "suggestion_insert", array($uid, $rid, $fid, $sadded));
			return $result;
		}
		return false;
	}
	...
	function duplicate_friend ($sender, $receiver) {
		$query = pg_prepare($db_conn, "find_duplicate", 'SELECT COUNT(sender) FROM friends WHERE sender = $1 AND receiver = $2');
		$result = pg_execute($db_conn, "find_duplicate", array($sender, $receiver));
		return ($result != 0);
	}
	
	function insert_to_friends ($sender, $receiver, $fadded) {
		if (if_int($sender) && if_int($receiver) && strtotime($fadded)) {
			if ($sender != $receiver && !duplicate_friend($sender, $receiver)) {
				$query = pg_prepare($db_conn, "friends_insert", 'INSERT INTO friends VALUES $1, $2, $3');
				$result = pg_execute($db_conn, "friends_insert", array($sender, $receiver, $fadded));
				return $result;
			}
		}
		return false;
